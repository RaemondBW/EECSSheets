...
\begin{itemize}
    ...
    \item Standard (as set by ISO, International Organization for Standardization and the IEC, International Electrotechnical Commission) of three challenge-response system.
    \item Non Standard with Symmetric Cryptography\\
        \begin{enumerate}
            \item \(\texttt{Bob} \rightarrow \texttt{Alice: }N_{B}\)
            \item \(\texttt{Alice} \rightarrow \texttt{Bob: }M,\texttt{MDC}(K_{AB},M,N_{B})\)\\
            \item \(\texttt{Bob reconstructs MDC}(K_{AB},M<N_{B}) \texttt{ and}\)\\
                \(\left\{\begin{array}{ll}
                        \texttt{accepts} & \texttt{if two MDCs are identical}\\
                        \texttt{rejects} & \texttt{otherwise}
                    \end{array}\right.\)
        \end{enumerate}
    \item Non Standard with Asymmetric Cryptography\\
        \begin{enumerate}
            \item \(\texttt{Bob} \rightarrow \texttt{Alice: } N_{B}\)
            \item \(\texttt{Alice} \rightarrow \texttt{Bob: sig}_{A}(M,N_{B})\)
            \item Bob verifies the signature using his nonce and\\
                \(\left\{\begin{array}{ll}
                        \texttt{accepts} & \texttt{if signature verification passes}\\
                        \texttt{rejects} & \texttt{otherwise}
                    \end{array}\right.\)
        \end{enumerate}
\end{itemize}
...
\begin{itemize}
    ...
    \item One of the ISO and IEC standards for mutual authentication is the "ISO public Key Three-Pass Mutual Authentication Protocol"\\
        \(\begin{array}{ll}
            \texttt{PREMISE:} & \texttt{A has a public key certificate Cert}_{A}\\
            \texttt{ } & \texttt{B has a public key certificate Cert}_{B}\\
            \texttt{GOAL} & \texttt{They achieve mutual authentication}
        \end{array}\)\\
        \begin{enumerate}
            \item \(B \rightarrow A: R_{B}\)
            \item \(A \rightarrow B: \texttt{ Cert}_{A},\texttt{ Token }AB\)
            \item \(B \rightarrow A: \texttt{ Cert}_{B}, \texttt{ Token }BA\)
        \end{enumerate}
        Here,
        \[\texttt{Token } AB = R_{A} || R_{B} || B || \texttt{sig}_{A}(R_{A}||R_{B}||B)\]
        \[\texttt{Token } BA = R_{B} || R_{A} || A || \texttt{sig}_{B}(R_{B}||R_{A}||A)\]
\end{itemize}

\subsubsection*{Needham-Schroeder password protocol}
% todo expand on this
\(\begin{array}{ll}
    \texttt{Premise: } & \texttt{User, U and Host, H have setup U's password}\\
    \texttt{ } & \texttt{entry } (ID_{u},f(P*u)) \texttt{where f is a one-way}\\
    \texttt{ } & \texttt{function; U memorizes password } P_u\\
    \texttt{Goal: } & \texttt{U logs in H using her/his password.}
\end{array}\)
\begin{enumerate}
...
\subsubsection*{S/Key Protocol}
\(\begin{array}{ll}
    \texttt{PREMISE:} & \texttt{User U and Host H have setup U's initial}\\
    \texttt{ } & \texttt{password entry }(ID_{U},f^{n}(P_{U}),n) \texttt{where f is a}\\
    \texttt{ } & \texttt{cryptographic hash function; U memorizes}\\
    \texttt{ } & \texttt{password } P_{U}\\
    \texttt{GOAL:} & \texttt{U authenticates to H without transmitting } P_{U}\\
    \texttt{ } & \texttt{in cleartext}
\end{array}\)
\begin{enumerate}
    \item \(U \rightarrow H: ID_{U}\)
    \item \(H \rightarrow U:c\), "Input Password"
    \item \(U \rightarrow H: Q=f^{c-1}(P_{U})\)
    \item H finds entry \((ID_{U},f^{c}(P_{U}),c)\) from its archive\\
        Access is granted if \(f(Q)=f^{c}(P_{U})\), and U's password entry is updated to\\
        \((ID_{U},Q,c-1)\)
\end{enumerate}
...
\begin{center}
\includegraphics[scale=.4]{ParallelSessionAttack}
\end{center}
Result: Malice has \(f^{c-2}(P_{U})\) which he can use for logging-ing in the name of U
...
\end{itemize}
\begin{center}
    \includegraphics[scale=.33]{KerberosExchange}
\end{center}
\begin{itemize}
...
Doing access control at the level of files and programs has grown dramatically in complexity over the years. Middleware is an attempt to provide authentication inside programs or databases.

\subsubsection*{Object Request Brokers (ORBs)}
an ORB is a software component that mediates communications between objects (object as in OOP).\\
Languages have been created to express security policies, this was specifically driven by the defence and rights management industies.

\subsubsection*{Sandboxing}
Starting with Java, another means to implement access control is through a software sandbox. Code is stuck in and executed in an interpreter, the Java Virtual Machine (JVM)

\subsubsection*{Virtualization}
Virtualization refers to systems that enable a single machine to emulate a number of machines independently. At the client end, virtualization allows people to run a host operating system on top of a guest (for example Linux on top of OSX).

\subsubsection*{Trusted Computing}
'Trusted Computing' was an initiative launched by Microsoft and a number of other tech companies to provide a more secure computer. The aim was to provide software and hardware add-ons to the PC architecture that would enable people to be sure that a given program was running on a machin with a given specification. Originially created to support Digital Rights Management.

\subsubsection*{Hardware Protection}
Is the idea of preventing one process from interfering with another, commonly called the protection problem. The confinement problem attepts to keep programs from communicating outward other than through their authorized channels The confinement problem attepts to keep programs from communicating outward other than through their authorized channels.
...
\begin{center}
\includegraphics[scale=.3]{StackSmash}
\end{center}
...
\subsubsection*{SQL Insertion Attacks}
These commonly arise when a careless webdeveloper passes user input to a back-end database without checking to see whether it contains SQL code.

\subsubsection*{Integer Manipulation Attack}
If you don't check for data sizes before writing, the integer manipulation attack uses an overflow, underflow, wrap-arround or truncation that can result from writing an inappropriate number of bytes to the stack.
...
\subsubsection*{User Interface Failures}
One of the earliest attacks was the Trojan Horse, a program that the administrator is invited to run and which will do some harm if he does so. Ex) a game that would check whether the player was the administrator, and if so create another administrator account with a known password.
